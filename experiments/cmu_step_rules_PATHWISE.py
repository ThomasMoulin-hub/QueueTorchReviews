...
def pathwise_cmu_step_rule(env_config, seed, num_iter, step_rule_name, alpha,
                           temp=0.01, T=1000, eval_T=10000):
    """pathwise_cmu with pluggable step rule instead of fixed normalized SGD."""
    torch.set_num_threads(1)
    ...
    avg_cost = evaluate_iterate_fast(avg_iterate[-1], env_config, eval_T=eval_T)
...
def _run_job(kwargs):
    torch.set_num_threads(1)
    return pathwise_cmu_step_rule(**kwargs)
